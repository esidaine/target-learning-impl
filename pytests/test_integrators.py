...
def test_firing_rate_dynamics_converge(tiny_network, tiny_batch, dendritic_effect):
    ...
    for i, pop in enumerate(tiny_network.populations):
        ...
        assert (
            pop.target_activation is not None
        ), f"pop[{i}].target_activation was never set"
        assert torch.allclose(pop.a_controlled, pop.target_activation, atol=1e-3), (
