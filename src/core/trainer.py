...
class Trainer:
    """Coordinate control optimization, plasticity, and training metrics."""
    ...
    def __init__(
        ...
        """Initialize a training coordinator for one network.

        Args:
            network (Network): Network whose weights and states are updated.
            controller (ControlMechanism): Controller used to optimize local signals.
            plasticity (Plasticity): Rule used to update forward weights.
            refresh_feedback (bool): Whether to refresh feedback after each update.
            log_every (int): Batch interval for feedback-alignment logging.
    ...
    def train_one_epoch(self, dataloader) -> tuple[float, float]:
        """Run one epoch of control optimization and local plasticity updates.

        Args:
            dataloader (Iterable): Batches of sensory inputs and target outputs.

        Returns:
            tuple[float, float]: Mean baseline loss and mean control magnitude.
        """
        ...
        for _, (sensory_inputs, target_y) in enumerate(dataloader):
            ...
            optimal_controls, metrics = self.controller.optimize_control_signal(
                sensory_inputs=sensory_inputs, target_y=target_y, network=self.network
            )
            ...
            with torch.no_grad():
                batch_c_mag = sum(
                    torch.abs(c).mean().item() for c in optimal_controls
                ) / len(optimal_controls)
            ...
            self.plasticity.update_weights(
                network=self.network, sensory_inputs=sensory_inputs
            )
            ...
            if self.step % self.log_every == 0 and metrics.final_control is not None:
                log_feedback_alignment(
                    self.network, global_control=metrics.final_control
                )
